...
reD1 = "(\d\d)/(\d\d)/(\d\d\d\d)"
reD2 = "(\d?\d)-(\d?\d)-(\d\d\d\d)"
reD3 = "(\d?\d) (janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|decembre) (\d\d\d\d)"

#reMois="(janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|decembre)"
...
def liste_dates_triplet(texte):
	...
	reD2 = "(\d{1,2})-(\d{1,2})-(\d{4})"
	reD3 = "(\d{1,2}) (janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|decembre) (\d{4})"
	m=re.findall("(?:(\d{2})/(\d{2})/(\d{4}))|(?:(\d{1,2})-(\d{1,2})-(\d{4}))|(?:(\d{2})/(\d{2})/(\d{4}))",texte)
	mReturn = [tuple([s1 for s1 in s ]) for s in m]
	return mReturn
